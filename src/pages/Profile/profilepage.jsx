...
const Profilepage = () => {
  ...
  const [input, setinput] = useState([]);  // user details
  const [value, setvalue] = useState(null);  // content to be shown in the area below Events/Teams/Requests tab
  const [formFilled, setFormFilled] = useState(null);
  const [showLoader, setShowLoader] = useState(false);
  const [loaderText, setLoaderText] = useState("");
  const [eventData, setEventData] = useState(null);   // eventName -> WA_link
    ...
    if (localStorage.getItem("loginData")) {
      const gettingData = async () => {
        ...
        const { data } = await axios.get(
          `http://localhost:8000/api/user/${localStorage.getItem("loginData")}`
        );

        console.log(data);

        const datas = await axios.get(`http://localhost:8000/api/events`);
